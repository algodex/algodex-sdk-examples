...
const getCancelOrderPromise = async (api, order, environment) => {
    ...
    const orderInput = checkAndGetInput(order.escrowAddress, orderbookEntry, order.version, order.ownerAddress, appId, order.type === 'buy');
...
const getAccountExistsFromIndexer = async (account, indexer) => {
    try {
        const accountInfo = await indexer.lookupAccountByID(account).do();
