...
jest.mock('axios', (req) => jest.fn((req) => {
    if (req.url.includes('assets') && req.url.includes('testnet')) {
      return {data: 
        {"ok":true,"rows":237,"data":[{"id":442424,"unix_time":1657428189,
        "price":0.068907007455,"priceBefore":0.068907007455,"price24Change":0,"isTraded":true},
        {"id":3333332,"unix_time":1657962386,"price":3.061931833692,"priceBefore":3.08962015105,
        "price24Change":-0.89617221549356,"isTraded":true}]}
      }
    } else if (req.url.includes('assets') && req.url.includes('app')) {
      // Mainnet
      return {data: 
        {"ok":true,"rows":237,"data":[{"id":333,"unix_time":1657428189,
        "price":0.05555,"priceBefore":0.068907007455,"price24Change":0,"isTraded":true},
        {"id":444,"unix_time":1657962386,"price":3.222,"priceBefore":3.08962015105,
        "price24Change":-0.89617221549356,"isTraded":true}]}
      }
    } else if (req.url.includes('tinyman') && req.url.includes('mainnet')) {
      //asset 0 is price of algo
     return {data: {"0":{"price":2}, "542132831":{"price":2.1e-05},"793124631":{"price":0.330026},"378382099":{"price":0.190918}}}
    } else if (req.url.includes('tinyman') && req.url.includes('testnet')) {
