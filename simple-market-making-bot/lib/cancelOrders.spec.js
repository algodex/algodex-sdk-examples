const {cancelOrders, convertCancelResultsToDBPromises}
...
test('converts cancel results', () => {
  ...
  const promises = convertCancelResultsToDBPromises(escrowDB, results, orders);
  expect(escrowDB.remove.mock.calls.length).toBe(1);
  const mockResults = escrowDB.remove.mock.calls[0][0];
  expect(JSON.stringify(mockResults)).toBe(JSON.stringify({'order': results[0][0]}));
});
...
test('executes promises', async () => {
  ...
  const results = 
    await cancelOrders(escrowDB, orders, [new Promise(resolve => resolve(promiseResults[0]))]);
