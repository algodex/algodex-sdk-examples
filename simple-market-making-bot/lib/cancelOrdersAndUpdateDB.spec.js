...
jest.mock('./cancelOrders', () => jest.fn((escrowDB, currentEscrows, cancelPromises) => {
...
test('can cancel orders and update DB', async () => {
  ...
  expect(getCancelPromisesMock.calls[0][0]).toEqual({"escrows":["algorand_address_1",
      ...
      expect(cancelOrdersMock.calls[0][1]).toEqual(['algorand_address_1', 'algorand_address_2']);
